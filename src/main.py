...
import math
import random
from db import init_db, get_settings, update_settings, get_best_score, update_best_score

init_db()
menu_volume, level_music_volume, effects_volume = get_settings()
pygame.init()
...
class Application:
    def __init__(self, screen):
        ...
        self.background = load_background(
            "data/images/bg.jpg", SCREEN_WIDTH, SCREEN_HEIGHT
        )
...
class Button:
    ...
    def save(self):
        menu_volume = self.sliders[0].get_current_value()
        level_music_volume = self.sliders[1].get_current_value()
        effects_volume = self.sliders[2].get_current_value()
        update_settings(menu_volume, level_music_volume, effects_volume)
        print(
            f"Settings saved: Menu Volume: {menu_volume}, Level Music Volume: {level_music_volume}, Effects Volume: {effects_volume}"
        )
...
class Slider:
    ...
    def draw(self, screen):
        ...
        text_rect = text_surface.get_rect(
            midtop=(SCREEN_WIDTH // 2, self.y_position - 40)
        )
        screen.blit(text_surface, text_rect)
        pygame.draw.rect(
            screen,
            GREY,
            (SLIDER_X, self.y_position, SLIDER_WIDTH, SLIDER_HEIGHT),
            border_radius=3,
        )
        knob_x = SLIDER_X + self.knob_pos - KNOB_SIZE // 2
        pygame.draw.circle(
            screen, RED, (knob_x, self.y_position + SLIDER_HEIGHT // 2), KNOB_SIZE
        )
    ...
    def _process_click(self, mouse_pos):
        ...
        if (
            mouse_pos[0] >= knob_x - KNOB_SIZE
            and mouse_pos[0] <= knob_x + KNOB_SIZE
            and mouse_pos[1] >= knob_y - KNOB_SIZE
            and mouse_pos[1] <= knob_y + KNOB_SIZE
        ) or (
            SLIDER_X <= mouse_pos[0] <= SLIDER_X + SLIDER_WIDTH
            and self.y_position <= mouse_pos[1] <= self.y_position + SLIDER_HEIGHT
        ):
...
class Scores:
    def __init__(self, screen, sp):
        ...
        self.images = [
            pygame.transform.scale(
                pygame.image.load(f"data/images/{str(i)}.png"),
    ...
    def draw(self):
        for i in range(len(self.sp)):
            rect = pygame.Rect(
                self.start_x + self.step_x * i, self.start_y, self.step_x, self.step_y
            )
...
class Bufs(pygame.sprite.Sprite):
    arrow_icon = pygame.transform.scale(
        pygame.image.load("data/images/arrow.png"), (100, 50)
    )
    heart_icon = pygame.transform.scale(
        pygame.image.load("data/images/heart.png"), (150, 100)
    )
...
class Bullet(pygame.sprite.Sprite):
    image = pygame.transform.scale(
        pygame.image.load("data/images/bullet.png"), (16, 32)
    )
...
class Enemy_bullet(pygame.sprite.Sprite):
    image = pygame.transform.scale(
        pygame.image.load("data/images/enemy_bullet.png"), (16, 32)
    )
...
class Enemy_2(GameObject):
    left_image = pygame.transform.scale(
        pygame.image.load("data/images/enemy_2_left.png"), (128, 72)
    )
    right_image = pygame.transform.scale(
        pygame.image.load("data/images/enemy_2_right.png"), (128, 72)
    )
    image = pygame.transform.scale(
        pygame.image.load("data/images/enemy_2.png"), (128, 72)
    )
...
class Enemy_Hight(GameObject):
    left_image = pygame.transform.scale(
        pygame.image.load("data/images/enemy_left_1.png"), (128, 72)
    )
    right_image = pygame.transform.scale(
        pygame.image.load("data/images/enemy_right_1.png"), (128, 72)
    )
...
class Ship(GameObject):
    left_image = pygame.transform.scale(
        pygame.image.load("data/images/resized_ship_left.png"), (256, 144)
    )
    right_image = pygame.transform.scale(
        pygame.image.load("data/images/resized_ship_right.png"), (256, 144)
    )
    shield_left_image = pygame.transform.scale(
        pygame.image.load("data/images/ship_shield_left.png"), (256, 144)
    )
    shield_right_image = pygame.transform.scale(
        pygame.image.load("data/images/ship_shield_right.png"), (256, 144)
    )
    shield_image = pygame.transform.scale(
        pygame.image.load("data/images/ship_shield.png"), (256, 144)
    )
...
class MainGameScreen:
    background_image = pygame.transform.scale(
        pygame.image.load("data/images/bg_play_image.jpg"),
        (SCREEN_WIDTH, SCREEN_HEIGHT),
    )
    hp = [
        pygame.transform.scale(pygame.image.load(f"data/images/hp_{i}.png"), (600, 300))
        for i in range(15, -1, -1)
    ]
    arrow_icon = pygame.transform.scale(
        pygame.image.load("data/images/arrow.png"), (200, 100)
    )
    heart_icon = pygame.transform.scale(
        pygame.image.load("data/images/heart.png"), (300, 200)
    )
    arrow_icon_gray = pygame.transform.scale(
        pygame.image.load("data/images/arrow_gray.png"), (200, 100)
    )
    heart_icon_gray = pygame.transform.scale(
        pygame.image.load("data/images/heart_gray.png"), (300, 200)
    )
    ...
    def __init__(self, screen):
        ...
        self.minus_hp = [
            pygame.mixer.Sound("data/sounds/отнимание_жизни.mp3"),
            pygame.mixer.Sound("data/sounds/отнимание_жизни_2.mp3"),
        ]
        ...
        self.heart_rect = self.heart_icon.get_rect(
            bottomright=(450, SCREEN_HEIGHT - 15)
        )
        self.scores = 0
        self.count_wawe = 0
        self.wawe_timer = 0
    ...
    def run(self):
        ...
        while running:
            ...
            if elapsed_time < self.message_lifetime:
                self.message_alpha = int(
                    255 - (elapsed_time / self.message_lifetime) * 255
                )
            else:
            ...
            for event in pygame.event.get():
                if event.type == QUIT:
                    ...
                    if self.arrow_rect.collidepoint(pos) and self.ship.count_speed > 0:
                        ...
                        self.heart_rect.collidepoint(pos) and self.ship.count_shield > 0
                    ):
                        self.ship.activate_shield(duration=10000)
                        self.ship.count_shield -= 1
                    else:
                        bullet = Bullet(
                            self.ship.rect.centerx - 20, self.ship.rect.centery - 5
                        )
            ...
            for enemy in self.enemy_group:
                if (
                    abs(enemy.rect.centerx - self.ship.rect.centerx) <= 30
                    and (pygame.time.get_ticks() - enemy.last_shot_time) > 500
                ):
                    self.enemy_bullet_group.add(
                        Enemy_bullet(enemy.rect.centerx - 5, enemy.rect.centery)
                    )
                    self.enemy_bullet_group.add(
                        Enemy_bullet(enemy.rect.centerx + 5, enemy.rect.centery)
                    )
    ...
    def run(self):
        ...
        while running:
            ...
            if self.scores > best_scores:
                ...
                msg = message(f"It's a record! Congratulations!!!", "Red", n=150)
...
def main_menu(screen):
    ...
    while running:
        ...
        mouse_pos = pygame.mouse.get_pos()
        if (
            start_x <= mouse_pos[0] <= start_x + 640
            and start_y <= mouse_pos[1] <= start_y + 310
        ):
            start = True
            settings = False
            ...
            settings_x <= mouse_pos[0] <= settings_x + 640
            and settings_y <= mouse_pos[1] <= settings_y + 310
        ):
            start = False
            settings = True
...
if __name__ == "__main__":
    ...
    one_fifth_screen_height = SCREEN_HEIGHT // 5
    sliders = [
        Slider("Звук меню", one_third_screen_height, initial_value=menu_volume),
        Slider(
            "Музыка уровня",
            one_third_screen_height + one_fifth_screen_height,
            initial_value=level_music_volume,
        ),
        Slider(
            "Эффекты",
            one_third_screen_height + 2 * one_fifth_screen_height,
            initial_value=effects_volume,
        ),
    ]
    start_x = 640
    ...
    background = pygame.transform.scale(
        pygame.image.load("data/images/menu_2.png"), (SCREEN_WIDTH, SCREEN_HEIGHT)
    ).convert()
    start_button = pygame.transform.scale(
        pygame.image.load("data/images/menu_2_start.png"), (SCREEN_WIDTH, SCREEN_HEIGHT)
    ).convert()
    settings_button = pygame.transform.scale(
        pygame.image.load("data/images/menu_2_settings.png"),
        (SCREEN_WIDTH, SCREEN_HEIGHT),
    ).convert()
    main_menu(screen)
